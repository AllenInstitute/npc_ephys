...
class LFP:
    ...
    def __post_init__(self):
        ...
        if self.traces.shape[0] != self.timestamps.shape[0]:
            raise ValueError('Mismatch in time dimension between traces and aligned timestamps')
        
        if self.traces.shape[1] != len(self.channel_ids):
            raise ValueError('Mismatch in channel dimension between traces and selected channels')

def _get_LFP_subsampled_output(LFP_subsampled_files: upath.UPath) -> tuple[zarr.core.Array, ...]:
...
def _get_LFP_channel_ids(LFP_channels: zarr.core.Array | list[str]) -> tuple[int, ...]:
    ...
    channel_ids = sorted([int("".join(i for i in channel if i.isdigit())) - 1 for channel in LFP_channels[:]])
    assert (
        m := min(channel_ids)
...
def _get_LFP_probe_result(probe: str, device_timing: npc_ephys.openephys.EphysTimingInfo, 
                          LFP_subsampled_directories: tuple[upath.UPath, ...], temporal_factor:int=2) -> LFP:
    probe_LFP_subsampled_directory = tuple(directory for directory in LFP_subsampled_directories if directory.is_dir() and probe in str(directory))
    if not probe_LFP_subsampled_directory:
        raise FileNotFoundError(f'No LFP subsampled results for probe {probe}. Check codeocean')
    
    probe_LFP_directory = probe_LFP_subsampled_directory[0]
    probe_LFP_traces, probe_LFP_samples, probe_LFP_channels = _get_LFP_subsampled_output(probe_LFP_directory)
    # time samples from output are evenly spaced at 1/1250, apply scale: (actual sampling rate / temporal factor) / 1250
    probe_LFP_aligned_timestamps = (probe_LFP_samples[:] / ((device_timing.sampling_rate / temporal_factor) / LFP_SUBSAMPLED_SAMPLING_RATE)) + device_timing.start_time
    probe_LFP_channel_ids = _get_LFP_channel_ids(probe_LFP_channels)

    return LFP(traces=probe_LFP_traces, timestamps=probe_LFP_aligned_timestamps, channel_ids=probe_LFP_channel_ids,
                         probe=probe, sampling_rate=device_timing.sampling_rate / temporal_factor)

def get_LFP_subsampled_results(session: str | npc_session.SessionRecord, 
                               device_timing_on_sync: Iterable[npc_ephys.openephys.EphysTimingInfo]) -> tuple[LFP, ...]:
    """
    ...
    session_LFP_subsampled_directories = npc_lims.get_LFP_subsampling_paths_from_s3(session)
    ...
    for device_timing in devices_LFP_timing:
        ...
        probe_LFP_subsampled_result = _get_LFP_probe_result(probe, device_timing, session_LFP_subsampled_directories)
